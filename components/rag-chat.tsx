...
import { Send, Bot, User, Sparkles, MessageCircle, Terminal } from 'lucide-react'
...
export function RagChat({ fullWidth = false }: RagChatProps) {
  ...
  return (
    <motion.div 
      className={cn(
        "flex flex-col rounded-2xl glass overflow-hidden border border-primary/20",
        fullWidth ? "h-[600px] w-full" : "h-[520px] w-full"
      )}
          ...
          {messages.length === 0 ? (
            ...
            >
              ...
              <h3 className="text-lg font-semibold text-foreground mb-2">
                Welcome! I&apos;m Jay&apos;s AI Assistant
              </h3>
              <p className="text-sm text-muted-foreground mb-6 max-w-md">
                Ask me anything about Jay&apos;s experience, skills, projects, education, or achievements. I&apos;m here to help you learn more!
              </p>
              <div className={cn(
                "flex flex-wrap gap-2 justify-center",
                fullWidth ? "max-w-2xl" : "max-w-sm"
              )}>
                {suggestedQuestions.slice(0, fullWidth ? 6 : 4).map((question, i) => (
                  <motion.button
                    key={i}
                    onClick={() => {
                      setInput(question)
                ...
                >
                  ...
                  >
                    {message.parts.map((part, index) => {
                      if (part.type === 'text') {
                        return <span key={index} className="whitespace-pre-wrap leading-relaxed">{part.text}</span>
                      }
                      return null
                    })}
                  </div>
